...
\newcommand{\versiondoi}{10.5281/zenodo.22837197}
...
{Santib\'a\~nez-Leal: Height provenance in open-data 3D urban reconstruction}
...
the LoD2 reference of the 3D BAG~\cite{peters} as a layer of its own. Every building also carries a land-cover
class from ESA WorldCover 2021~\cite{worldcover}, the $10$~m class sampled at a point inside its footprint, which
the tool uses to colour and aggregate buildings by land cover; unlike the layers, this attribute has no provenance
entry in the place manifests.
...
method; the per-building land-cover class of Section~\ref{sec:method} is not recorded there. The figure script under \path{manuscripts/geo-reconstruction/figures/} reads \path{mq.json} in the data folder
...
the result. The scenes were baked with \texttt{geoscena} at commit \texttt{ab0bbf8} (14 July 2026), pinned in
\path{data-pipeline/requirements-bake.txt}; its 0.1.0 release on PyPI lacks the population, Open Buildings, LoD2,
Sentinel-2 and environment fetchers that the bake uses. The open layers are fetched under their own licences and are not redistributed.
