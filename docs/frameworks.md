...
|---|---|---|
| geoscena ([CAOS_GeoScena](https://github.com/fsantibanezleal/CAOS_GeoScena)) | fetching, per-building fusion and the height ladder, terrain / building / road meshing, the SceneBundle and its manifest | source newer than the 0.1.0 release on PyPI; not pinned here yet |
...
| geopandas, requests, shapely | `gen_admin`: geoBoundaries units, clipping, the environment calls | not pinned here yet |
