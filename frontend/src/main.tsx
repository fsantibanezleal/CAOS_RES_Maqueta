...
const config: ShellConfig = {
  ...
  footer: {
    provenance: {
      en: 'Data: a multi-modal fusion assembled offline by the geoscena pipeline from open public geodata: building footprints (Overture, ODbL), terrain (Copernicus GLO-30), land cover (ESA WorldCover), water / green / rail (OpenStreetMap), population (GHS-POP), 2.5D heights (Google Open Buildings), LoD2 ground truth (3DBAG), satellite multispectral NDVI/NDWI/NDBI (Copernicus Sentinel-2), solar potential (PVGIS), climate normals (Open-Meteo ERA5), admin sub-areas (geoBoundaries, CC-BY) and Chilean socio-economic indicators (Data Observatory). Each layer carries its source, license and fetch date. Code: MIT. Nothing is invented.',
      es: 'Datos: una fusión multi-modal ensamblada offline por el pipeline geoscena a partir de geodatos públicos abiertos: huellas de edificios (Overture, ODbL), relieve (Copernicus GLO-30), cobertura de suelo (ESA WorldCover), agua / verde / vías (OpenStreetMap), población (GHS-POP), alturas 2.5D (Google Open Buildings), verdad LoD2 (3DBAG), multiespectral satelital NDVI/NDWI/NDBI (Copernicus Sentinel-2), potencial solar (PVGIS), clima (Open-Meteo ERA5), subáreas administrativas (geoBoundaries, CC-BY) e indicadores socioeconómicos chilenos (Observatorio de Datos). Cada capa lleva su fuente, licencia y fecha. Código: MIT. Nada es inventado.',
    },
    disclaimer: {
      ...
      es: 'Una reconstrucción de investigación: las alturas se etiquetan por procedencia (medida vs inferida); el relieve de un DSM global de 30 m queda algo alto en centros densos; los índices satelitales son proxies de superficie de una sola escena; solar/clima son casi constantes por AOI (agregados por comuna); las capas no comerciales se marcan por escena.',
    },
  },
};
