...
bundle.write("out/santiago")   # terrain.glb, buildings.glb, roads.glb, manifest.json`}</code>
      ...
      <p>
        {es
          ? 'maquetalab hornea cada lugar del registro (40, en tres tiers, mas el caso de Santiago completo) llamando a geoscena: descarga las modalidades (Overture por su CLI oficial; GLO-30 y WorldCover por lectura COG con ventana via GDAL vsicurl, sin credenciales; OSM por Overpass; GHS-POP por COG; alturas 2.5D de Open Buildings por raster teselado; verdad LoD2 de 3DBAG por OGC API), fusion (la escalera de alturas, muestreo de cobertura y de elevacion base), malla (TIN de relieve, extrusion, cintas, celdas de poblacion) y exportacion (glb + manifiesto), con compresion meshopt de entrega. Es tolerante a capas: un area natural sin edificios hornea su relieve y registra el hueco, sin fallar.'
          : 'maquetalab bakes each registry place (40, in three tiers, plus the full-Santiago case) by calling geoscena: fetch the modalities (Overture via its official CLI; GLO-30 and WorldCover via COG windowed reads through GDAL vsicurl, keyless; OSM via Overpass; GHS-POP via COG; Open Buildings 2.5D heights via tiled raster; 3DBAG LoD2 ground truth via OGC API), fuse (the height ladder, land-cover and base-elevation sampling), mesh (terrain TIN, extrusion, ribbons, population cells) and export (glb + manifest), with meshopt delivery compression. It is layer-tolerant: a natural area with no buildings bakes its terrain and records the gap, without failing.'}{' '}
