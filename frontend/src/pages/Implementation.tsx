      ...
      <Figure caption={t('Two lanes: a heavy offline bake (fetch, fuse, mesh, analytical layers, meshopt) and a light static replay. The manifest is the contract between them.',
        ...
        <svg className="mq-fig-svg" viewBox="0 0 640 200" role="img" preserveAspectRatio="xMidYMid meet">
          ...
          {[
            ...
            { x: 300, en: 'mesh (TIN, extrude)', es: 'malla (TIN, extrusión)' },
            { x: 452, en: 'analytical layers', es: 'capas analíticas' },
          ].map((s, i) => (
            <g key={i}>
          ...
          <text className="f-mu" x="18" y="172">{t('static replay lane (React + Three.js, read-only)', 'carril de reproducción estática (React + Three.js, solo lectura)')}</text>
          <text className="f-ac" x="18" y="186">{t('layers · filter · colour · select · aggregate by sub-area', 'capas · filtro · color · selección · agregar por subárea')}</text>
      ...
      <ul>
        <li>
          ...
          {t('each fetcher returns geometry/raster PLUS a LayerProvenance (source, URL, license, fetch date, method). No layer, and no analytical value, enters without it.',
            'cada fetcher devuelve geometría/raster más una LayerProvenance (fuente, URL, licencia, fecha, método). Ninguna capa, ni valor analítico, entra sin ella.')}
        </li>
        <li>
          ...
          {t('the SceneBundle manifest.json, a versioned schema carrying layers, modalities, the environment block and credits. A TypeScript type mirrors it in the frontend, so a schema drift fails the web build.',
            'el manifest.json del SceneBundle, un esquema versionado que lleva capas, modalidades, el bloque de ambiente y créditos. Un tipo TypeScript lo espeja en el frontend, así una deriva de esquema rompe el build.')}
        </li>
      </ul>
      <Callout variant="strong" title={t('Honesty by construction', 'Honestidad por construcción')}>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'El frontend (React + Three.js sobre el shell compartido @fasl-work/caos-app-shell) es una proyección de solo lectura de los bundles precalculados y auditados. Carga cada .glb con GLTFLoader, registra el decodificador meshopt, aplica los materiales de la escena, y hace picking por raycast leyendo el atributo de vértice _featureid para resolver un edificio y leer sus atributos fusionados. El panel de control maneja el coloreo, los filtros por atributo, la herramienta de estadísticas de área, el drapeado satelital, y el coropleto de agregar por área administrativa. El bucle de render está pausado por defecto y se detiene con la pestaña oculta, así no hay bomba de cómputo.',
        )}{' '}
        <Cite id="gltf" />
      </p>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'Producto estático: el pipeline precalcula cada lugar offline en la máquina local (datos crudos en un volumen fuera de git, con un cache de descarga en disco para que los re-precalculados sean rápidos), y el sitio se sirve como archivos estáticos puros por nginx sobre HTTPS. Como los bundles precalculados son grandes (los núcleos metropolitanos llegan a decenas de MB cada uno), Maqueta vive en el box ml/heavy (más disco), no en el box de producción limitado por memoria, y despliega vía un directorio de staging más un intercambio atómico para que el sitio en vivo nunca quede a medio escribir.',
        )}
      </p>
    </div>
  );
    ...
    <section className="page-body prose">
      ...
      <SubTabs
        ...
        tabs={[
          ...
          { id: 'app', label: t('The app (replay)', 'La app (reproducción)'), content: APP },
