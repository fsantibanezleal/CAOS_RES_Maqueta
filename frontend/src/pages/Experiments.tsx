...
export default function Experiments() {
  ...
  const DESIGN = (
    <div className="mq-method">
      ...
      <h4>{t('Metrics', 'Metricas')}</h4>
      <ul>
        <li><b>{t('Measured-height %', '% de altura medida')}</b>: {t('share of buildings whose height is a real measurement (Overture/OSM), not an inference.', 'proporcion de edificios cuya altura es una medicion real (Overture/OSM), no una inferencia.')}</li>
        <li><b>{t('Layer coverage', 'Cobertura de capas')}</b>: {t('how many modalities were available and fused for the place (terrain, buildings, roads, context, population, analytical layers).', 'cuantas modalidades hubo y se fusionaron para el lugar (relieve, edificios, calles, contexto, poblacion, capas analiticas).')}</li>
        <li><b>{t('Mesh budget', 'Presupuesto de malla')}</b>: {t('triangles and delivered megabytes, the cost of the scene after meshopt compression.', 'triangulos y megabytes entregados, el costo de la escena tras la compresion meshopt.')}</li>
        <li><b>{t('Ground-truth error', 'Error vs verdad de terreno')}</b>: {t('where an open LoD2 exists, the fused-vs-authoritative height RMSE, MAE and bias (see Benchmark).', 'donde existe un LoD2 abierto, el RMSE, MAE y sesgo de altura fusion-vs-autoritativo (ver Benchmark).')}</li>
      </ul>
  ...
  const RESULTS = bench ? (
    <div className="mq-method">
      <p>{t('The fusion result per place: layers fused, mesh budget, and the measured-height share. Sortable-by-eye; the sharp regional variation in the measured share is the honest headline.',
        'El resultado de fusion por lugar: capas fusionadas, presupuesto de malla, y la proporcion de altura medida. La fuerte variacion regional en la proporcion medida es el titular honesto.')}</p>
      <div className="mq-tablewrap" style={{ maxHeight: 480, overflowY: 'auto' }}>
        <table className="mq-table">
          <thead>
            <tr>
              <th>{t('Place', 'Lugar')}</th><th>{t('Tier', 'Tier')}</th><th>{t('Layers', 'Capas')}</th>
              <th>{t('Triangles', 'Triangulos')}</th><th>MB</th><th>{t('Measured height', 'Altura medida')}</th>
            </tr>
          </thead>
          <tbody>
  ...
  return (
    <section className="page-body prose">
      ...
      <Tabs
        ...
        tabs={[
          { id: 'design', label: t('Design & metrics', 'Diseno y metricas'), content: DESIGN },
