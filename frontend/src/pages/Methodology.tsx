...
export default function Methodology() {
  ...
  return (
    <section className="page-body prose">
      ...
      <p>
        {es
          ? 'Continuas (cobertura, índices multiespectrales NDVI/NDWI/NDBI de Sentinel-2, potencial solar PVGIS, temperatura, viento y precipitación) se muestrean sobre cada edificio/celda por estadística zonal (media); categóricas (cobertura de suelo, función, geología) por mayoría; superposiciones vectoriales (uso de suelo, cuencas, inundación) se recortan al AOI. El potencial solar y las normales climáticas son casi constantes en un AOI de pocos km, así que se registran una vez por lugar y se muestrean por centroide de comuna para agregarlas por sub-área. Cada valor viaja con su procedencia: la escalera de alturas (sección 2) es el caso base de una etiqueta de fuente por atributo, de modo que lo medido y lo inferido nunca se confunden. La desagregación de datos agregados (p.ej. población sobre estructuras) sigue el mapeo dasimétrico; mezclar un DSM con un DTM nacional exige cuidar el datum vertical.'
          : 'Continuous modalities (land cover; the Sentinel-2 multispectral indices NDVI/NDWI/NDBI; solar potential from PVGIS; temperature, wind and precipitation from ERA5) are sampled onto each building/cell by zonal statistics (mean); categorical ones (land cover, function, geology) by majority; vector overlays (land use, watersheds, flood) are clipped to the AOI. Solar potential and climate normals are near-constant across a few-km AOI, so they are recorded once per place and re-sampled at each comuna centroid to aggregate by sub-area. Every value travels with its provenance: the height ladder (section 2) is the base case of a per-attribute source tag, so measured and inferred are never conflated. Disaggregating aggregate data (e.g. population over structures) follows dasymetric mapping; mixing a DSM with a national DTM requires vertical-datum care.'}{' '}
