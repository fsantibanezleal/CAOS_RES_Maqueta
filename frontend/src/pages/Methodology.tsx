...
export default function Methodology() {
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'Maqueta hace fusión de datos multi-modal a nivel de características (feature-level): no combina píxeles crudos ni decisiones finales, sino que alinea modalidades heterogéneas (huellas vectoriales, DSM raster, cobertura categórica, densidad de población, alturas 2.5D, modelos LoD2, índices multiespectrales satelitales, normales de solar y clima, indicadores socioeconómicos por comuna) en un único marco métrico local y luego las combina por edificio o por celda. El mapa de alturas es el punto de entrada y el andamio geométrico; cada otra modalidad se registra sobre él.',
        )}{' '}
      ...
      <p>
        {t(
          ...
          'Un Área de Interés es un centro WGS84 más un semilado en metros. Cada capa se expresa en una proyección Acimutal Equidistante local centrada en el centroide del AOI, en metros, origen (0,0) en el centro. En extensiones de pocos kilómetros la distorsión es muy inferior a un metro, y el visor trabaja directo en metros. En exportación glTF se aplica una sola conversión a Y-arriba, así cada .glb comparte una orientación y una escena Three.js carga las capas sin corrección por app.',
        )}
      </p>
      <Equation
        tex={String.raw`(x_{\text{glTF}},\,y_{\text{glTF}},\,z_{\text{glTF}}) = (\text{east},\ \text{up},\ -\text{north})`}
        caption={t('The single Y-up convention shared by every baked layer.', 'La única convención Y-arriba que comparte cada capa precalculada.')}
      />
      <Fig
        viewBox="0 0 520 210"
        caption={t('Heterogeneous modalities registered into one local metric frame, then combined per building/cell.',
          'Modalidades heterogéneas registradas en un único marco métrico local, luego combinadas por edificio/celda.')}
      >
        {[
          { y: 24, en: 'Footprints (Overture)', es: 'Huellas (Overture)' },
          { y: 52, en: 'Terrain DSM (GLO-30)', es: 'Relieve DSM (GLO-30)' },
          { y: 80, en: 'Land cover / population', es: 'Cobertura / población' },
          { y: 108, en: 'Satellite indices, solar, climate', es: 'Índices satelitales, solar, clima' },
        ].map((r, i) => (
          <g key={i}>
        ...
        <text className="f-tx-b" x="336" y="100">{t('Local metric frame', 'Marco métrico local')}</text>
      ...
      <Callout variant="honest" title={t('Assumptions', 'Supuestos')}>
        {t('AEQD distortion is negligible only for few-km AOIs; a metro-scale AOI (11 km) still stays sub-metre. Fusion is feature-level: sources keep their identity and provenance, they are not blended into pixels.',
          'La distorsión AEQD es despreciable solo para AOIs de pocos km; un AOI a escala metropolitana (11 km) sigue bajo el metro. La fusión es a nivel de características: las fuentes conservan su identidad y procedencia, no se mezclan en píxeles.')}
      </Callout>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'La altura del edificio es el andamio, así que se resuelve con una escalera de procedencia explícita: para cada huella gana la primera regla que da un valor positivo, y la fuente usada se registra como etiqueta por edificio, de modo que lo medido y lo inferido nunca se confunden.',
        )}
      </p>
      <Equation
      ...
      <Fig
        ...
        caption={t('The ladder: the first rung that fires sets the height and stamps the source tag.',
          'La escalera: el primer peldaño que dispara fija la altura y estampa la etiqueta de fuente.')}
      >
        {[
        ...
        <text className="f-mu" x="8" y="106">{t('the per-place source mix is baked and reported on the Benchmark', 'la mezcla de fuentes por lugar se precalcula y se reporta en el Benchmark')}</text>
      </Fig>
      <Callout variant="honest" title={t('Assumptions', 'Supuestos')}>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'La grilla del DEM se convierte en una red irregular de triángulos (TIN) adaptativa: densa donde el relieve es complejo, dispersa donde es plano. Partiendo de las esquinas, el refinamiento voraz inserta por rondas los puntos de mayor error vertical y re-triangula hasta que el peor error cae bajo un umbral.',
        )}
      </p>
      <Equation
        ...
        caption={t('z is the true elevation; z-hat is the current TIN linear interpolation. Batched top-K insertion turns thousands of triangulations into dozens.',
          'z es la elevación real; z-gorro es la interpolación lineal de la TIN actual. La inserción por lotes (top-K) pasa de miles de triangulaciones a decenas.')}
      />
      <Fig viewBox="0 0 520 150" caption={t('A uniform grid (left) refined to an adaptive TIN (right): triangles cluster on the slope.',
        'Una grilla uniforme (izq.) refinada a una TIN adaptativa (der.): los triángulos se agrupan en la pendiente.')}>
        <g>
    ...
    <div className="mq-method">
      ...
      <Equation
        ...
        caption={t('Every prism rises from the terrain-sampled base b to b+h; the footprint area a is carried as an attribute.',
          'Cada prisma sube desde la base b tomada del relieve hasta b+h; el área de huella a viaja como atributo.')}
      />
      <Fig viewBox="0 0 520 150" caption={t('Footprint to closed prism; a per-vertex id keeps each building selectable + attributed.',
        'De la huella al prisma cerrado; un id por vértice mantiene cada edificio seleccionable y con atributos.')}>
        ...
        <g transform="translate(380,0)">
          ...
          <text className="f-tx" x="22" y="80">{t('area, function,', 'área, función,')}</text>
      ...
      <Callout variant="honest" title={t('Assumptions / limitations', 'Supuestos / limitaciones')}>
        {t('Footprints are extruded as flat-roof prisms; genuine roof shapes appear only in the authoritative LoD2 ground-truth layer (3DBAG). Height is a single value per building, not a per-storey model.',
          'Las huellas se extruyen como prismas de techo plano; las formas reales de techo aparecen solo en la capa de verdad LoD2 (3DBAG). La altura es un valor único por edificio, no un modelo por planta.')}
      </Callout>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'La capa satelital analítica encuentra la escena Sentinel-2 L2A reciente con menos nubes sobre el AOI a través de la API STAC de Earth Search (sin auth), lee por ventana las bandas visibles + NIR + SWIR de los COGs sentinel-cogs, las reproyecta a una grilla WGS84 sobre el AOI, y deriva tres índices de diferencia normalizada muestreados en cada centroide de huella: NDVI (vegetación), NDWI (agua / humedad) y NDBI (construido / impermeable).',
        )}{' '}
      ...
      <Equation
        ...
        caption={t('Bands B04 (Red), B03 (Green), B08 (NIR, 10 m), B11 (SWIR, 20 m). Each index is in [-1, 1].',
          'Bandas B04 (Rojo), B03 (Verde), B08 (NIR, 10 m), B11 (SWIR, 20 m). Cada índice está en [-1, 1].')}
      />
      <p>
        {t('These ride as per-building attributes, so they become colour-by, filter and aggregate-by-comuna layers exactly like height or floor count. A true-colour and a false-colour composite are also produced for the optional terrain drape.',
          'Estos viajan como atributos por edificio, así que se vuelven capas de colorear, filtrar y agregar por comuna igual que la altura o el número de pisos. También se producen un compuesto de color real y uno de falso color para el drapeado opcional sobre el relieve.')}
      </p>
      <Fig viewBox="0 0 520 160" caption={t('Reflectance bands to normalized-difference indices, sampled per footprint centroid.',
        'Bandas de reflectancia a índices de diferencia normalizada, muestreados por centroide de huella.')}>
        {[
        ...
        <text className="f-mu" x="360" y="58">{t('per-building index ramp', 'rampa de índice por edificio')}</text>
        <defs>
      ...
      <Callout variant="honest" title={t('Assumptions / limitations', 'Supuestos / limitaciones')}>
        {t('A single-date scene: a cloud over one block leaves a few centroids null (recorded, never interpolated). Indices are surface proxies, not calibrated biophysical variables; a small building samples the reflectance of its 10-20 m block, not its roof alone.',
          'Una escena de una fecha: una nube sobre una manzana deja algunos centroides nulos (registrados, nunca interpolados). Los índices son proxies de superficie, no variables biofísicas calibradas; un edificio pequeño muestrea la reflectancia de su bloque de 10-20 m, no solo su techo.')}
      </Callout>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'La capa de ambiente adjunta el contexto geofísico del lugar: potencial de energía solar de PVGIS (rendimiento PV anual de 1 kWp conectado a red e irradiación horizontal global anual) y normales climáticas del archivo ERA5 de Open-Meteo (temperatura media / más fría / más cálida anual, viento medio diario máximo, precipitación anual). Ambos son llamadas JSON sin auth.',
        )}{' '}
      ...
      <Equation
        ...
        caption={t('Daily ERA5 values reduced to annual normals; solar + climate are sampled once per place and again at each comuna centroid for sub-area aggregation.',
          'Valores diarios ERA5 reducidos a normales anuales; solar + clima se muestrean una vez por lugar y otra en cada centroide de comuna para la agregación por subárea.')}
      />
      <Fig viewBox="0 0 520 150" caption={t('One AOI centroid queries PVGIS + Open-Meteo; the same is done per comuna centroid to vary by sub-area.',
        'Un centroide de AOI consulta PVGIS + Open-Meteo; lo mismo por centroide de comuna para variar por subárea.')}>
      ...
      <Callout variant="honest" title={t('Assumptions / limitations', 'Supuestos / limitaciones')}>
        {t('ERA5 is ~25 km, so within one city these are a single climatological point, not a gradient; that is exactly why they are stored per place and aggregated per comuna, where they do vary. Wind is a proxy (mean of daily maxima), and it is one recent year (2023), not a 30-year normal.',
          'ERA5 es ~25 km, así que dentro de una ciudad son un único punto climatológico, no un gradiente; por eso se guardan por lugar y se agregan por comuna, donde sí varían. El viento es un proxy (media de máximos diarios), y es un año reciente (2023), no una normal de 30 años.')}
      </Callout>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'La agregación por subárea une los edificios a unidades administrativas oficiales y resume cualquier atributo por unidad. Los límites vienen de geoBoundaries al nivel más fino que da dos o más unidades en el AOI (ADM3 comuna/barrio/distrito, luego ADM2 provincia/estado, luego ADM1 región). Cada edificio se asigna a su unidad por punto-en-polígono con descarte por bbox sobre su centroide; la herramienta colorea un coropleto y llena una tabla ordenada.',
        )}{' '}
      ...
      <Equation
        ...
        caption={t('Building averages per unit u over the buildings B_u; c is the normalized position on the colour ramp. Environment / socio-economic layers use the unit-level value directly, not a building mean.',
          'Promedios de edificios por unidad u sobre los edificios B_u; c es la posición normalizada en la rampa. Las capas de ambiente / socioeconómicas usan el valor a nivel de unidad, no un promedio de edificios.')}
      />
      <Fig viewBox="0 0 520 150" caption={t('Buildings point-in-polygon into comunas, then a choropleth + ranked table across three layer bands.',
        'Edificios por punto-en-polígono en comunas, luego un coropleto + tabla ordenada en tres bandas de capas.')}>
        <g>
      ...
      <Callout variant="honest" title={t('Assumptions / limitations', 'Supuestos / limitaciones')}>
        {t('Cities whose tight AOI sits inside a single unit (e.g. one US county) have no official sub-areas; the draw-polygon Area Statistics tool covers custom sub-areas everywhere. Data Observatory indicators are joined by comuna name and reported as raw counts, not normalized by population or area.',
          'Las ciudades cuyo AOI ajustado cae dentro de una sola unidad (p.ej. un condado de EE.UU.) no tienen subáreas oficiales; la herramienta de dibujar polígono cubre subáreas a medida en todas partes. Los indicadores del Observatorio de Datos se unen por nombre de comuna y se reportan como conteos crudos, no normalizados por población o área.')}
      </Callout>
    ...
    <div className="mq-method">
      <p>
        {t(
          ...
          'Cada capa se escribe como glTF binario (.glb) válido; luego el pipeline aplica compresión meshopt (EXT_meshopt_compression) como paso de entrega, sin pérdida para el id por vértice ni los atributos por edificio, típicamente ~60% más liviano. Para AOIs de pocos km basta un .glb por capa; las escenas a escala metropolitana (11 km, ~179k edificios) son el caso pesado, comprimidas y transmitidas con un proxy LoD liviano primero. 3D Tiles (py3dtiles) es la vía de escape si un lugar excede el presupuesto.',
        )}{' '}
        <Cite id="gltf" />
      </p>
      ...
      <Callout variant="note" title={t('Contract', 'Contrato')}>
        {t('meshopt is lossless for the _featureid attribute (picking) and extras.features (per-building data); the manifest is a typed contract mirrored in the frontend, so a schema drift fails the web build.',
          'meshopt es sin pérdida para el atributo _featureid (selección) y extras.features (datos por edificio); el manifiesto es un contrato tipado espejado en el frontend, así una deriva de esquema rompe el build.')}
      </Callout>
  ...
  return (
    <section className="page-body prose">
      <h2>{t('Methodology', 'Metodología')}</h2>
      <p className="mq-lead">
        {t('How open public geodata becomes an interrogable 3D area: feature-level fusion into a local metric frame, a height-provenance ladder, an adaptive terrain mesh, building extrusion, the analytical layers (satellite indices, solar + climate, sub-area aggregation), and web delivery. One method family per tab, each with its equations, a figure and its references.',
          'Cómo los geodatos públicos abiertos se vuelven un área 3D interrogable: fusión a nivel de características en un marco métrico local, una escalera de procedencia de alturas, una malla de relieve adaptativa, extrusión de edificios, las capas analíticas (índices satelitales, solar + clima, agregación por subárea) y entrega web. Una familia de métodos por pestaña, cada una con sus ecuaciones, una figura y sus referencias.')}
      </p>
      <SubTabs
        ariaLabel={t('Method families', 'Familias de métodos')}
        orientation="vertical"
        tabs={[
          { id: 'fusion', label: t('Fusion & frame', 'Fusión y marco'), content: FUSION },
          ...
          { id: 'satellite', label: t('Satellite indices', 'Índices satelitales'), content: SATELLITE },
          { id: 'environment', label: t('Solar & climate', 'Solar y clima'), content: ENVIRONMENT },
          { id: 'subarea', label: t('Sub-area aggregation', 'Agregación por subárea'), content: SUBAREA },
